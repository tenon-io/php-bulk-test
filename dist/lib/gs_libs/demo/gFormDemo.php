...
<div id="content" role="application">
    ...
    $warnAttrs = array('class' => 'warn_msg', 'role' => 'alert');
    ...
    $form->dateDropSet('Birthday', $birthdaySettings, 'birthday', array('class' => 'dateSet'), array('class' => 'inline'));


    $monthdrop['name'] = "exp_month";
    $monthdrop['id'] = "exp_month";
    $monthdrop['default_label'] = "Month";
    $monthdrop['label'] = "Month";

    if (!isset($form->presets['exp_month'])) {
        $form->addPreset('exp_month', date("m"));
    }
    $expDaySettings['monthdrop'] = $monthdrop;

    $yeardrop['name'] = "exp_year";
    $yeardrop['id'] = "exp_year";
    $yeardrop['default_label'] = "Year";
    $yeardrop['label'] = "Year";
    $yeardrop['start'] = date("Y");
    $yeardrop['end'] = date("Y") + 8;

    if (!isset($form->presets['exp_year'])) {
        $form->addPreset('exp_year', date("Y"));
    }
    $expDaySettings['yeardrop'] = $yeardrop;

    $expDaySettings['order'] = 'month|year';

    $form->dateDropSet(_('Expiration'), $expDaySettings, 'expiration', array('class' => 'dateSet'), array('class' => 'inline'));
